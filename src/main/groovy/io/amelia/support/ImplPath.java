 ...
 * @see Path
 */
public interface ImplPath<Self extends io.amelia.support.ImplPath> extends Path
{
	...
	default Self normalize()
	{
		if ( getNameCount() == 0 )
			return create();
		io.amelia.support.Namespace ns = io.amelia.support.Namespace.empty();
	...
	default Self resolve( Path other )
	{
		if ( other.isAbsolute() )
			return create( other );
		io.amelia.support.Namespace ns = io.amelia.support.Namespace.of( getNames() );
