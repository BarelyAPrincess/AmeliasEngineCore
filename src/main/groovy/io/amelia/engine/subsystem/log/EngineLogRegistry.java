...
public class EngineLogRegistry
{
	...
	static
	{
		...
		try
		{
			UtilityIO.forceCreateDirectory( StorageEngine.getPath( StorageEngine.PATH_LOGS ) );
		}
		catch ( IOException e )
		{
			e.printStackTrace();
	...
	{
		Path logPath = StorageEngine.getPath( StorageEngine.PATH_LOGS ).resolve( filename + ".log" );
		try
		{
			if ( Files.exists( logPath ) )
			{
				if ( archiveLimit > 0 )
					UtilityIO.gzFile( logPath, StorageEngine.getPath( StorageEngine.PATH_LOGS ).resolve( new SimpleDateFormat( "yyyy-MM-dd_HH-mm-ss" ).format( new Date() ) + "-" + filename + ".log.gz" ) );
	...
	{
		Stream<Path> result = Files.list( StorageEngine.getPath( StorageEngine.PATH_LOGS ) ).filter( path -> path.toString().toLowerCase().endsWith( suffix.toLowerCase() ) );
