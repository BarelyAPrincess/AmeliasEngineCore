...
import io.amelia.engine.events.RunlevelEvent;
import io.amelia.engine.injection.Libraries;
import io.amelia.engine.injection.MavenReference;
...
import io.amelia.engine.users.Users;
...
public class EngineCore
{
	...
	{
		return devMeta != null && "0".equals( devMeta.getBuildNumber() ) || ConfigRegistry.config.getBoolean( ConfigKeys.DEVELOPMENT_MODE );
	}
	...
	private static void onRunlevelChange() throws ApplicationException.Error
	{
		...
		if ( currentRunlevel == Runlevel.STARTUP )
		{
			...
			entityNull = Users.createVirtualUser( nullUuid );
			entityRoot = Users.createVirtualUser( rootUuid );
	...
	public static void prepare()
	{
		...
		try
		{
			...
			String depends = UtilityIO.resourceToString( "dependencies.txt" );
	...
	public static void setRunlevel( @Nonnull Runlevel runlevel, @Nullable String reason )
	{
		...
		if ( !mainLooper.isThreadJoined() && app.isPrimaryThread() || mainLooper.isHeldByCurrentThread() )
			...
			throw new ApplicationException.Startup( "Application has CRASHED!" );
