...
import io.amelia.engine.looper.queue.AbstractQueue;
...
public abstract class AbstractLooper<Q extends AbstractQueue>
{
	...
	public void joinLoop() throws ApplicationException.Error
	{
		...
		try
		{
			...
			for ( ; ; )
			{
				...
				{
					EngineCore.L.warning( "[" + getName() + "] Time ran backwards! Did the system time change?" );
					lastPolledMillis = 0L;
				}
				...
				{
					if ( loopStartMillis - lastWarningMillis >= 15000L && ConfigRegistry.config.isTrue( ConfigKeys.WARN_ON_OVERLOAD ) )
					{
						EngineCore.L.warning( "[" + getName() + "] Can't keep up! Did the system time change, or is it overloaded?" );
	...
	public class LooperControl
	{
		...
		{
			EngineCore.getExecutorParallel().execute( () -> {
