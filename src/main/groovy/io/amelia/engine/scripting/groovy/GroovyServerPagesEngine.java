			...
			if ( script == null )
			{
				...
				while ( fullFileIndex < source.length() )
				{
					...
					if ( startIndex > -1 )
					{
						...
						if ( endIndex == -1 )
							throw new ScriptingException.Error( ReportingLevel.E_PARSE, String.format( "Found starting marker '%s' at line %s, expected close marker '%s' not found.", activeMarker.getStart(), UtilityStrings.countMatches( output.toString(), '\n' ) + 1, activeMarker.getEnd() ) );
